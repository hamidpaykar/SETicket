...
export const FileUpload = ({
  ...
  const handleFileChange = (newFiles: File[]) => {
    ...
    const acceptedFiles = newFiles.filter(isValidFile);
    const rejectedFiles = newFiles.filter(file => !isValidFile(file));

    if (rejectedFiles.length > 0) {
      rejectedFiles.forEach((file) => {
        toast.error(`Unsupported file: ${file.name}`, {
          description: "Please upload a JPG, PNG, PDF, or Excel file.",
        });
      });
    }
    ...
    <div className="w-full" {...getRootProps()}>
      ...
      >
        ...
        <div className="flex flex-col items-center justify-center">
          ...
          <p className="relative z-20 font-sans text-xs text-neutral-400 dark:text-neutral-500 mt-1">
            Allowed file types: JPG, PNG, PDF, Excel
          </p>
